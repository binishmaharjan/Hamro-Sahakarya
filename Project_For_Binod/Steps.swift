...
//  Project_For_Binod
//
//  Created by Maharjan Binish on 2020/03/18.
...
final class Steps: CustomDebugStringConvertible {
  ...
  func label(_ label: String = "", handler: @escaping (Steps) -> Void) -> Steps {
    ...
    if label.contains(label) {
      Dlog("Steps(\(name)) is duplicated. This will be ignored")
    } else {
      labels.insert(label)
    }
  ...
  private func next(index: Int) {
    ...
    if let item = items[optional: current] {
      Dlog("Steps(\(name)) => label(\(item.label))")
      item.block(self)
    } else {
      Dlog("Steps(\(name)) => index(\(current)) is out of range!")
    }
  }
